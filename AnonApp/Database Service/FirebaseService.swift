...
class FirebaseService: NSObject {
    ...
    func getActiveUserQuips(uid:String, completion: @escaping ([Quip],[String:Any], Double)->()){
        setCurrentTimeForUser(uid:uid)
        var currentTime:Double = 0
        var myScores:[String:Any] = [:]
        var myTopScores:[Quip] = []
        let query1 = ref.child("M/\(uid)/q").queryOrdered(byChild: "s")
        ...
        query1.observeSingleEvent(of: .value, with: {(snapshot)   in
            ...
            while let rest = enumerator.nextObject() as? DataSnapshot {
                        ...
                        completion(myTopScores,myScores, currentTime)
